...
| 技术模块    | 选型                               | 理由                                                                                |
| ------- | -------------------------------- | --------------------------------------------------------------------------------- |
| 多模态语义理解 | **Qwen-VL**, **MobileVLM-v2**    | Qwen-VL 提供强大的图文语义解析能力，适用于复杂指令理解与视频描述生成；MobileVLM-v2 专为移动端优化，适合边缘设备部署，保障推理效率与能耗控制。 |
| 语音识别    | **Whisper**                      | 支持多语言、多口音识别，鲁棒性高，适合长音频剪辑与字幕生成，提升口播类视频处理能力。                                        |
| 视频补全与生成 | **E2FGVI**, **Stable Diffusion** | E2FGVI 支持目标移除与自然背景补全，提升画面连贯性；Stable Diffusion 支持风格化图像生成与虚拟背景构建，增强视觉创意。            |
| 目标检测与分割 | **YOLO11**, **SAM2**             | YOLO11 精准快速，适配移动场景；SAM2 提供像素级分割，便于复杂遮罩生成与后期处理。                                    |
| 视频处理    | **MobileFFmpeg**, **GPUImage**   | 支持实时滤镜、裁剪与编码，MobileFFmpeg 覆盖主流格式与编解码器，GPUImage 提升渲染性能，满足低延迟预览与边剪边看。               |

### 6.1.2 推理与部署工具链
| 技术组件    | 选型                                                                    | 理由                                                                   |
| ------- | --------------------------------------------------------------------- | -------------------------------------------------------------------- |
| 模型格式标准化 | **ONNX**                                                              | 提供跨框架兼容性，简化从 PyTorch 到移动平台（如 iOS CoreML，Android NNAPI）的部署流程，降低维护复杂度。 |
| 模型加速推理  | **TensorRT**                                                          | 支持 FP16/INT8 量化与图优化，提升 GPU 推理速度，减少移动端延迟，满足实时剪辑响应需求。                  |
| 移动端优化   | **MobileVLM 系列**, **TensorRT + ONNX**, **轻量化模型裁剪工具链（如 Neural Magic）** | 适应不同硬件条件下部署，兼顾性能与能耗，支持端侧计算需求的平衡。                                     |
...
| 预览与图像渲染 | **GPUImage**, **GL React Native**          | 支持高性能图像渲染与滤镜应用，满足剪辑实时反馈需求。                                         |

### 6.1.4 数据与存储选型
| 模块   | 选型           | 理由                                       |
| ---- | ------------ | ---------------------------------------- |
| 本地数据 | **SQLite**   | 支持离线数据访问，轻量可靠，适合存储本地项目配置、草稿、用户偏好等信息。     |
| 云端同步 | **Firebase** | 实现跨设备同步、账号绑定与分享；支持用户认证、存储与权限管理；适配全球部署场景。 |
